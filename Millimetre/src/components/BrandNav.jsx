  ...
  return (
    <div className="flex">
      <div className="fixed top-0 right-0 left-0 bg-gray-800  z-50 h-12 w-[100vw]  flex items-center justify-between px-4">
        ...
        <Notification className="mr-12" />
      </div>
      <nav
        className={`mt-12 h-[110vh]  ${
          open
            ? "fixed top-0 left-0   z-40 bg-gradient-to-r from-green-200   "
            : "hidden"
        }  w-64`}
      >
        <ul>
              ...
              {!item.subPaths ? (
                <Link
                  ...
                  className={`block px-4 duration-150 text-gray-700  ${
                    MenuName === item.text ? "text-green-600 " : "none"
                  } outline-none ${openAccount ? "py-2" : "py-3"}`}
                >
